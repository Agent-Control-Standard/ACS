...
ACS conformance, at ACS-Core or any profile above it, is a technical measure. It guarantees that records are well-formed, authenticated, and honored — not that what a record says is true. In particular, conformance cannot establish that the system producing it is connected to human intent at all: a deployment whose policies were authored by automation and whose every ASK decision is answered by an automated service is fully conformant and produces a complete, intact audit chain, and nothing in the protocol can distinguish it from a human-governed one. A conformant record may state that a human approved an action or that a delegation applied; conformance does not make those claims true. Verifying them requires a process outside this specification, and conformance alone cannot satisfy any requirement for human authorization or oversight. This risk grows with conformance quality: the more authoritative and complete the conformance evidence appears, the more readily it will be accepted as a replacement for the institutional accountability and human intent it cannot attest.

Concretely, requirements that do locate authority in humans and institutions — the NIST AI RMF GOVERN function (documented human roles and executive accountability for AI risk: GOVERN 2.1, 2.3, 3.2; MAP 3.5), MAESTRO Layer 7 operational governance, and OWASP LLM06:2025's guidance that authorization live in downstream systems rather than in model decisions — are not satisfied by ACS conformance at any tier. ACS can record evidence that such processes ran; it cannot stand in for them.

Verification of the governance claims that fall outside this specification may be pursued external to ACS: that the bootstrap — Guardian policy, Intent scope, and Approver configuration — was publicly adopted, endorsed, or authorized by an accountable human with the authority to do so, and that each Approver traces to a human principal through a documented delegation chain. Authority absent at initialization cannot be added afterward from within the protocol; where it exists, ACS-Trace and ACS-Audit are well-suited to record it.
